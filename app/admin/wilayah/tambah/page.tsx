        ...
        <div className="bg-slate-900 px-8 py-6 flex items-center justify-between relative overflow-hidden">
          ...
          <div className="flex items-center gap-4 relative z-10">
            ...
            <div className="text-left">
              ...
              <p className="text-slate-400 text-[14px] font-bold tracking-tighter mt-1 italic">Pilih kecamatan/desa dari referensi geolokasi</p>
            </div>
          </div>
        </div>
          ...
          <div className="w-full bg-white p-6 md:p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase flex items-center gap-2">
                    <Map size={12} className="text-blue-600" /> Kecamatan *
                  </label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase flex items-center gap-2">
                    <Map size={12} className="text-blue-600" /> Desa/Kelurahan *
                  </label>
                  <div className="relative">
                    <select
                      required
              ...
              <div className="lg:col-span-3 space-y-8 py-4 border-y border-slate-50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {SEKTOR_LIST.map((sektor) => (
                        <button key={sektor} type="button" onClick={() => toggleSektor(sektor, 'sesuai')} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all border-2 ${selectedUsahaSesuai.includes(sektor) ? 'bg-blue-600 border-blue-600 text-white shadow-md' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-blue-200'}`}>
                          {sektor}
                        </button>
                      ))}
                    </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {SEKTOR_LIST.map((sektor) => (
                        <button key={sektor} type="button" onClick={() => toggleSektor(sektor, 'kajian')} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all border-2 ${selectedPerluKajian.includes(sektor) ? 'bg-amber-500 border-amber-500 text-white shadow-md' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-amber-200'}`}>
                          {sektor}
                        </button>
                      ))}
                    </div>
                ...
                {selectedUsahaSesuai.length > 0 && (
                  <div className="space-y-4 animate-in slide-in-from-top-2 duration-500">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-2"><Wallet size={14} className="text-blue-600" /> Detail Perizinan Per Sektor</label>
              ...
              <div className="flex justify-end pt-6 border-t border-slate-50">
                <Button type="submit" disabled={loading || loadingMaster} className="w-full md:w-80 h-14 bg-blue-600 hover:bg-slate-900 text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-blue-200 transition-all flex gap-3 active:scale-95">
